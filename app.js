const DATA = {
  ...
  "boardGuide": [
    ...
    },
    {
      ...
      "boardTexture": "superwet",
      "boardTextureLabel": "超ウェット"
    },
    {
      "id": "FG013",
      "board": "987ss",
...
let state = { mode: "home", order: [], index: 0, score: 0, total: 0, answered: false, selectedHand: null, currentStreak: 0, bestStreak: 0, unlockedRanks: [], chips: 2500, collection: [], chargeCount: 0 };
...
function renderReference(mode){
  ...
  setTextSafe("#modeTitle", modeTitle(mode));
  setTextSafe("#progress", "-");
  setTextSafe("#score", "-");
  try{
    ...
    console.error("renderReference failed", mode, e);
    if(ref){
      ref.innerHTML = `<section class="card"><h2>表示エラー</h2><p>${mode} の表示中に問題が発生しました。</p><pre>${String(e.message || e)}</pre></section>`;
    }
  }
}
...
function getLabProgress(){try{state.chips=Number(localStorage.getItem("sharkLabChips")||2500);state.collection=JSON.parse(localStorage.getItem("sharkLabCollection")||"[]");state.chargeCount=Number(localStorage.getItem("sharkChargeCount")||0);}catch(e){state.chips=2500;state.collection=[];}}
...
function getChargeCount(){return Number(localStorage.getItem("sharkChargeCount")||state.chargeCount||0);}
function chargeMessage(n){
 const arr=["課金ありがとうございます！楽しんでください！","追加課金ありがとうございます！ガチャ運アップを祈っています！","もう一回！？ありがとうございます！","だんだん慣れてきましたね。","研究熱心で素晴らしいです。","本当に課金ボタン好きですね。","そろそろ自力でチップ集めませんか？","サメ先生「ん？」","サメ先生「また来たのか」","ご利用は計画的に。","課金ボタンが手に馴染んできましたね。","研究所の財務担当がざわついています。","このペース、なかなかの才能です。","チップの流れが激しくなってきました。","サメ先生「そのチップ、どこへ消えているんだ？」","ガチャ台があなたを覚えました。","研究所の入口で顔パスになりました。","これはもう日課ですね。","ガチャ依存度：中","破産に注意。","レンジ表より課金ボタンを見てないか？","サメ先生「一回、深呼吸しよう」","研究所から見守り対象に指定されました。","ガチャの期待値を計算したことはありますか？","君はもう止まれない。","🏆 課金王。ここまで押した人は初めてです。","LEGEND称号解放：スポンサー","研究所を買収しました。","🦈「もう好きなだけ引いてくれ」"];
 if(n<=arr.length)return arr[n-1]; if(n<50)return"研究所のチップ補充班が忙しくなってきました。"; if(n<100)return"サメ先生「君の覚悟は伝わった」"; if(n<200)return"研究所の壁に名前が刻まれました。"; return"🦈「もう好きなだけ引いてくれ」";
}
function chargeChips(){
 const n=getChargeCount()+1; state.chargeCount=n; localStorage.setItem("sharkChargeCount",String(n));
 state.chips=Number(state.chips||0)+1000; saveLabProgress();
 const chips=document.querySelector("#labChips"); if(chips)chips.textContent=Number(state.chips||0).toLocaleString();
 const msg=document.querySelector("#chargeMessage"); if(msg)msg.innerHTML=`<div class="charge-msg"><b>${chargeMessage(n)}</b><span>+1000 チップ / 課金回数 ${n}回</span></div>`;
}
  ...
  document.querySelector("#refArea").innerHTML=`
    <section class="card gacha-room museum">
      ...
      <div class="gacha-machine premium-machine">
        ...
        <div class="gacha-actions"><button class="pull-btn" onclick="pullGacha()">1000チップで引く</button><button class="charge-btn" onclick="chargeChips()">💰 課金 +1000</button></div><div id="chargeMessage" class="charge-message-slot"></div>
...
function renderBoardDictionary(){
 const area=document.querySelector("#refArea");
 const filters=[["all","全部"],["dry","ドライ"],["semi","セミドライ"],["wet","ウェット"],["superwet","超ウェット"],["mono","モノトーン"],["pair","ペアボード"]];
 area.innerHTML=`<section class="card board-dict-premium"><h2>🃏 ボード事典</h2><p class="note">ボードの質感で絞り込めます。</p><div class="board-filter-bar">${filters.map(([k,l])=>`<button class="${k==="all"?"active":""}" onclick="filterBoards('${k}', this)">${l}</button>`).join("")}</div><div class="board-list-premium">${DATA.boardGuide.map(b=>`<article class="board-lab-card" data-cat="${b.boardTexture||"dry"}"><div class="board-lab-head"><div>${boardCards(b.board)}</div><span>${b.boardTextureLabel||b.category||"Board"}</span></div><div class="board-theme"><b>ボード特徴</b><p>${b.boardTheme||b.texture||""}</p></div><div class="hit-grid hit-grid-v14"><div class="hit-card overpair"><b>オーバーペア</b><p>${b.overpairStrategy||""}</p></div><div class="hit-card"><b>トップヒット</b><p>${b.hitStrategy?.topHit||b.topHitStrategy||""}</p></div><div class="hit-card"><b>セカンドヒット</b><p>${b.hitStrategy?.secondHit||b.secondHitStrategy||""}</p></div><div class="hit-card"><b>ボトムヒット</b><p>${b.hitStrategy?.bottomHit||b.bottomHitStrategy||""}</p></div><div class="hit-card"><b>ノーヒット</b><p>${b.hitStrategy?.noHit||b.noHitStrategy||""}</p></div></div><div class="board-risk-grid"><div><b>よく負ける相手</b><p>${b.commonThreats||"-"}</p></div><div><b>${b.dangerTurnsLabel||"ターンに落ちたら要注意のカード"}</b><p>${b.dangerTurns||"-"}</p></div></div>${sharkBox(b.sharkComment||b.sharkTip||"ボードごとの特徴を見よう。")}</article>`).join("")}</div></section>`;
}
function filterBoards(cat,btn){
 document.querySelectorAll(".board-filter-bar button").forEach(b=>b.classList.remove("active")); if(btn)btn.classList.add("active");
 document.querySelectorAll(".board-lab-card").forEach(card=>{const ok=cat==="all"||card.dataset.cat===cat; card.style.display=ok?"block":"none";});
}
